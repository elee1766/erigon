...
func processRewardsAndPenaltiesPostAltair(s *state.BeaconState) (err error) {
	...
	eligibleValidators := s.EligibleValidatorsIndicies()
	...
	for validatorIndex, validator := range s.Validators() {
		for i := range weights {
			if s.IsUnslashedParticipatingIndex(previousEpoch, uint64(validatorIndex), i) {
	...
	for _, index := range eligibleValidators {
		...
		for flagIdx := range weights {
			if s.IsUnslashedParticipatingIndex(previousEpoch, index, flagIdx) {
				if !s.InactivityLeaking() {
		...
		if !s.IsUnslashedParticipatingIndex(previousEpoch, index, int(beaconConfig.TimelyTargetFlagIndex)) {
...
func processRewardsAndPenaltiesPhase0(s *state.BeaconState) (err error) {
	beaconConfig := s.BeaconConfig()
	if s.Epoch() == beaconConfig.GenesisEpoch {
		return nil
	}
	eligibleValidators := s.EligibleValidatorsIndicies()
	// Initialize variables
	...
	for _, index := range eligibleValidators {
		...
		if s.InactivityLeaking() {
			...
			if validators[index].Slashed || !validators[index].IsPreviousMatchingTargetAttester {
				...
				if state.DecreaseBalance(s.BeaconState, index, validators[index].EffectiveBalance*s.FinalityDelay()/beaconConfig.InactivityPenaltyQuotient); err != nil {
					return err
				}
			}
		}
...
func ProcessRewardsAndPenalties(s *state.BeaconState) error {
	if s.Epoch() == s.BeaconConfig().GenesisEpoch {
		return nil
	}
