...
func processAttestationPhase0(s *state.BeaconState, attestation *cltypes.Attestation) ([]uint64, error) {
	...
	isCurrentAttestation := data.Target.Epoch == s.Epoch()
	...
	epochRoot, err := s.GetBlockRoot(attestation.Data.Target.Epoch)
	if err != nil {
		return nil, err
	}
...
func processAttestation(s *state.BeaconState, attestation *cltypes.Attestation, baseRewardPerIncrement uint64) ([]uint64, error) {
	...
	beaconConfig := s.BeaconConfig()
	// Prelimary checks.
	if (data.Target.Epoch != currentEpoch && data.Target.Epoch != previousEpoch) || data.Target.Epoch != s.GetEpochAtSlot(data.Slot) {
